    ...
    const [getUserQuery, {data:user_data}] = useLazyQuery(GET_USER_FOR_MESSAGE,{
        ...
        onCompleted: () => {
            if(user_data){
                ...
                setReportContent("We have received an report about a user for sending message");
            }
        }
    });
        ...
        <div style={{bottom: isMessageBlockOpen ? "250px" : "0px"}} className="messageWindow">
            <div className="messageWindowHeader">
                ...
                <div className="cardToolWindow" style={{left: "100px", top: "5px"}}>
                    ...
                    {report && <ReportWindow reportContent={reportContent} user={user} closeWindow={() => {
                        setReport(false)
                    }}/>}
                </div>}
            </div>
            ...
            <div className="messageInputWrapper">
                <input placeholder={updatedUser.blocked?"You have been blocked" : "Type your message"} disabled={updatedUser.blocked} value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyDown={sendMessage}>
                </input>
            </div>
        </div>
    )
